...
static int PRECEDENCE[] = {1, 1, 2, 2, -1, -1};  // 우선 순위
...
char *process_op(char op, char *pos) {
    if (is_empty(operator_stack) || op == '(') {   // if operator_stack is empty
        ...
        if (precedence(op) > precedence(top_op)) { // if operator_stack top stack is lower than parameter op
            ...
            while (!is_empty(operator_stack) &&
                   precedence(op) <= precedence(top_op)) { // while operator stack not empty ...
                ...
                pos += 2;     // blank space
                if (!is_empty(
                        operator_stack)) {           // doing 'while' statement, operator_stack is not empty top_op is peek.
